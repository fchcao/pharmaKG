...
> **重要说明（AutoDL 服务器）**: 在 AutoDL 服务器上，`pharmakg-api` conda 环境已经预配置完成，包含了所有必需的依赖。您可以直接激活该环境跳过环境创建步骤：
>
> ```bash
> conda activate pharmakg-api
> ```
>
> 如果依赖有更新或需要重新安装，请执行后续步骤。
...
conda create -n pharmakg-api python=3.11 -y
conda activate pharmakg-api
```

**或使用 venv（如果 conda 不可用）：**
...
```bash
source /root/autodl-tmp/pj-pharmaKG/api/activate_env.sh
```
